...
def test_resolve_pricing_for_known_model() -> None:
    pricing = resolve_model_pricing("gpt-5.5")

    assert pricing is not None
    assert pricing.resolved_model == "gpt-5.5"
    assert pricing.input_per_1m_usd == 5.0
    assert pricing.cached_input_per_1m_usd == 0.5
    assert pricing.output_per_1m_usd == 30.0
    assert pricing.long_context_threshold_tokens == 272000
    assert pricing.long_context_input_per_1m_usd == 10.0
    assert pricing.long_context_cached_input_per_1m_usd == 1.0
    assert pricing.long_context_output_per_1m_usd == 45.0
